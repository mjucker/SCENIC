c----------------------------------------------------------------------c
C  17.03.06        LAST MODIFICATION 04.07.07     UHS: mtaskb DATA   D
c-----------------------------------------------------------------------
C
...
c
     ...
     &                  fbjac,fgparp,fsigbs,fphv,glm,bgradg,gparp,
...
C..OUTPUT IN UNIT 38 FOR WAVE FIELD CALCULATIONS
     ...
     &                fsigbs(l,i)
      ...
      do i=1,ni
       shalf=0.5*(s(i)+s(i-1))
       write(37,822)
     $      shalf,-ftp(i),-fpp(i),pth(i),-ci(i),-cj(i),-cip(i),-cjp(i)
       write(38,823)
     $      shalf,-ftp(i),-fpp(i),-ci(i),-cj(i),-cip(i),-cjp(i)
       
      enddo
      write(38,'(3i6)')ni,njk,nj
      do i=1,ni
         do jk=1,njk
            write(38,823) bjac(jk,i),gssu(jk,i),gppl(jk,i),gtpl(jk,i)
     $           ,gttl(jk,i),gstl(jk,i),gssl(jk,i)
         enddo
      enddo
      write(38,'(2i6)')ni,lmnb
      do i=1,ni
         do l=1,lmnb
            write(38,831)mb(l),nb(l),fr(l,i),fz(l,i),fphv(l,i),fbsq(l,i)
     $           ,fsigbs(l,i)
         enddo
      enddo

 820  format(1x,2i6)
 821  format(1x,2i8,1p8e14.6)
 822  format(1x,1p8e14.6)
 823  format(1x,1p7e14.6)
 831  format(1x,2i8,1p5e14.6)
