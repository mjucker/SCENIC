...
c
        ...
        if (ns.gt.ni .or. nper.gt.npert)then
           print*,'ns,ni,nper,npert',ns,ni,nper,npert
           stop
        endif
c
        if (mod(nj,nskipp).ne.0)then
           print*,'nj,nskipp',nj,nskipp
           stop
        endif
            ...
            do 302 k=1,nk
              ...
              argp = twopif * (k-1)/nk
               do 300 j=1,nj
                 ...
                 cosp = cos(arg)
                 sinp = sin(arg)
                 cosc = cd*cos((j-1)*twopi/nj)
                 sinc = cd*sin((j-1)*twopi/nj)
                 x(ks,i,j)  = rt(jk,i) * cosp
                 y(ks,i,j)  = rt(jk,i) * sinp
                 z(ks,i,j)  = zt(jk,i)
                 xc(ks,i,j)  = (rt(jk,i)+cosc) * cosp
                 yc(ks,i,j)  = (rt(jk,i)+cosc) * sinp
                 zc(ks,i,j)  = zt(jk,i)+sinc
                 p(ks,i,j)  = pt(jk,i)
                 u(ks,i,j)  = ut(jk,i)
                 d(ks,i,j)  = dn(jk,i)
                 b(ks,i,j)  = bt(jk,i)
...
c
 ...
 335      end do
             ...
             xc(ks,0,1) = (1.5*xax(1)  - 0.5*xax(2) ) * dt
             yc(ks,0,1) = (1.5*yax(1)  - 0.5*yax(2) ) * dt
             zc(ks,0,1) = (1.5*zax(1)  - 0.5*zax(2) ) * dt
...
c
        do 405 i=0,ns
           do 402 ks=1,kf-1
             ...
             xc(ks,i,nj+1)  = xc(ks,i,1)
             yc(ks,i,nj+1)  = yc(ks,i,1)
             zc(ks,i,nj+1)  = zc(ks,i,1)
...
c--     WRITE OUTPUT.                                           -------c
c
        open(unit=45,file='Fields.45')
        open(unit=90,file='Boundary.45')
        open(unit=91,file='Coils.45')
        nistop = min(ns,nimax)
        nrp = 1 + nistop / nskipr
        npp = 1 + nj / nskipp  
        ntp = 1 + (kstop - kstart)!/nskipt
        write (45,1006) ntp+1, nrp, npp
        write (90,1009) ntp+1, npp
        write (91,1009) ncoils, npp
c$$$        write (46,1006) ntp, nrp, npp
c$$$        write (47,1006) ntp, nrp, npp
c$$$        write (48,1006) ntp, nrp, npp
 1006   format (3i4)
 1009   format (2i4)
        xx = 0.
        xy = 0.
        ...
        do 503 i=0,nistop,nskipr
           do 500 j=1,nj+1,nskipp
              do 505 ks = kstart,kstop,nskipt
                 write (45,1008) x(ks,i,j),y(ks,i,j),z(ks,i,j)
     $                ,b(ks,i,j),p(ks,i,j),u(ks,i,j),d(ks,i,j)
                 if(i==nistop)
     $                write (90,1008) x(ks,i,j),y(ks,i,j),z(ks,i,j)
     $                ,b(ks,i,j),p(ks,i,j),u(ks,i,j),d(ks,i,j)
c$$$                 write (46,1008) x(ks,i,j),y(ks,i,j),z(ks,i,j),p(ks,i,j)
c$$$                 write (47,1008) x(ks,i,j),y(ks,i,j),z(ks,i,j),u(ks,i,j)
c$$$                 write (48,1008) x(ks,i,j),y(ks,i,j),z(ks,i,j),d(ks,i,j)
 505          end do
              write (45,1008) x(kstart,i,j),y(kstart,i,j),z(kstart,i,j)
     $             ,b(kstart,i,j),p(kstart,i,j),u(kstart,i,j)
     $             ,d(kstart,i,j)
              if(i==nistop)
     $             write (90,1008)
     $             x(kstart,i,j),y(kstart,i,j),z(kstart,i,j)
     $             ,b(kstart,i,j),p(kstart,i,j),u(kstart,i,j)
     $             ,d(kstart,i,j)
 500       end do
 503    end do
cmj coils
        nskiptc=(kstop-kstart+1)/ncoils
        i=nistop
        do ks = kstart,kstop,nskiptc
           do j=1,nj+1,nskipp
              write (91,1010) xc(ks,i,j),yc(ks,i,j),zc(ks,i,j),-1.
           end do
        end do
c     
        close(45)
 ...
 1008   format(1p7e16.8)
 1010   format(1p4e16.8)
        stop
        end program Equil
C-----------------------------------------------------------------------
...
C
 ...
 711  format(3i6)
      if (mnmax.gt.mns)then
         print*,'mnmax,mns',mnmax,mns
         stop
      endif
      ns = nsp - 1
      dth = 1. / nj
